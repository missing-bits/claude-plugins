...
- **Content input:** the developer's own Python repositories are the content input for skill authoring. Every skill-content task begins with an ask-the-developer step for those repositories — NEVER invent personal preferences. Contested points are decided by the developer's stated preferences; everything else follows community consensus: PEPs, the PyPA packaging guide, official tool docs (uv, ruff, pytest, pyright, typer, FastAPI, pydantic).
...
This is a prerequisite, not optional: ask the developer for the paths or URLs of their Python repositories to use as content input (the spec names author repositories as the source for contested points). Record the answer for reuse by Tasks 3–7 in this session. Do NOT author any standard the repositories or the developer's answers do not support — community-consensus sources (PEPs, ruff docs) fill the rest.
