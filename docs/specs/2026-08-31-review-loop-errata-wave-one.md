...
- hit dismissed 2026-08-31 — eight sites in the shipped spec and plan still describe the pre-errata design; counter: both carry `status: implemented` and record what shipped in 0.14.0, and the spec's architect verdict certifies a body an `integrity:` hash covers, so a body edit would un-certify text a reviewer did read; `revises:` carries the pointer instead
...
- hit fixed 2026-09-02 — this document's own bare `- dismissed 2026-08-31` line predated the `hit dismissed` shape its wave went on to define; the token is added, and nothing else on the line changes
