...
- **Label and API name serve different audiences.** Label is user-facing
  (page layouts, reports, list views) and freely renamed via the
  Translation Workbench; API name is referenced by every formula,
  validation rule, flow, Apex class, and integration — decide it
  carefully at creation, since renaming it later means fixing every
  reference.
- **No abbreviations, ever, in the API name** — `Total_Amount__c`, not
  `Tot_Amt__c`.
- **Boolean (checkbox) fields read as an assertion**: `Is_` or `Has_`
  plus the condition — `Is_Active__c`, `Has_Signed_Contract__c` — never
  a bare noun (`Active__c` is ambiguous between "is active" and
  "activation date").
- One field-naming convention (PascalCase-with-underscores,
  no-abbreviation) applies uniformly across every field type — text,
  number, currency, date, picklist, formula, lookup, master-detail.
...
stating at minimum: what the object/field is *for*, and *who or what
writes it* (a user role, an integration, a flow, a formula). A
description that only repeats the label (`Total_Amount__c` — "The total
amount") satisfies the letter but not the purpose — write it for the
next person who opens Setup with no other context.

**A missing description is a review finding**, with no "custom fields
...
- **Record type vs a picklist-driven variation** — the question is
  ...
  - A record type introduced only to gate a handful of picklist values
    per "type", with no layout/sharing/process difference, is
    over-modeling — the picklist field alone would do the job with far
...
- **Naming**: descriptive, underscore-separated, no numbered prefixes —
  ...
  person nothing about what the rule checks.
- **User-actionable error messages.** The message names the field at
  fault (when field-level placement doesn't already point to it) and
  states what to change — not just that something is wrong.
  "Amount is required when Stage is Closed Won" tells the user what to
  do; "Invalid data" does not.
- **Every validation rule has an explicit bypass strategy**, named in
  the rule itself (a comment in the description field) or in the
  object's documentation — never an implicit "nobody thought about it."
  The house pattern is a **Custom Permission check**: a Custom Permission
  ...
  lacks the permission), granted via a permission set only to the
  integration/data-load users who legitimately need to bypass it. This
  skill covers only the check-in-formula mechanism; *who* gets the
  permission set assigned is `salesforce-security-model`'s concern.
- A validation rule with no bypass path at all forces every future
  legitimate exception (a data correction, a migration load) through an
...
like any other component, so a new environment gets the configuration
automatically instead of needing it reseeded by hand.
...
- **No duplicated inline value lists across objects.** Two fields
  ...
  the single place that addition happens.
...
silent wrong-record bug, waiting for the next environment.
